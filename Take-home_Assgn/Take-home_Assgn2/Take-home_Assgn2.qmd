...
Having a glimpse at our data, the relevant column(s) that we need for our task is `DESA_KELUR` which represents the village/sub districts in Jakarta.
...
Based on the output, we have discovered that there are 14 missing values in our dataset which may affect our analysis. Thus, I removed all missing values in the relevant column(s), using the code chunk below.
...
We can observe that the assigned coordinates system is WGS 84 which is not appropriate for this dataset. As this is an Indonesian-specific geospatial dataset, I will be using [EPSG:23845.](https://epsg.io/23845)
...
Yay, we have successfully managed to transform our dataset to the appropriate coordinates system!

**removal of outer islands**

Let visualise our data!
...
Looking at our data, once again, there are 3 attributes which represents the divisions of Jakarta. They are `KAB_KOTA` (City), `KECAMATAN` (District) and `DESA_KELUR` (Village). Removing the outer islands by the City would help us to remove all the districts and villages within it, so I proceeded to check the unique values of it.

```{r}
unique(jkt2019$"KAB_KOTA")
```

From this, we can observe that all the values has "JAKARTA" in it except "KEPULAUAN SERIBU"(Thousand Island). The code chunk below highlights the different cities in Jakarta.
...
Examining DKI Jakarta Vaccination File History website where we were suppose to get our data, some of my observations were:
...
-   Entire page is in Indonesian
    ...
    -   Alternative solutions for translation can be found [here](https://www.wikihow.com/Translate-a-Web-Page).
...
-   Many instances of missing data!
    ...
    -   E.g 2) Certain months such as July 2021, August 2021, February 2022 were missing the record of the date written in the table and its respective data
        ...
        -   Upon further research and some of my assumptions, missing data can be due to:
            ...
            -   Data not recorded as there is no available data due to the day being a holiday

                -   For e.g 28 February 2022 record and data is missing and that day so happens to be an Indonesian major holiday called [Isra Mi'raj](https://publicholidays.co.id/isra-miraj/). Upon further [research](https://www.3ecpa.co.id/resources/indonesia-public-holidays/isra-miraj/#:~:text=They%20walk%20in%20the%20city,Indonesia%20during%20Isra%20Mi'raj.), Indonesians celebrate that day by gathering at their local mosque for prayers and/or will be walking in the city going to the city hall while chanting the Salawat. Thus, there is a high possibility that people will not get vaccinated on this specific day.

                -   For reference, the list of holidays for [2021](https://www.officeholidays.com/countries/indonesia/2021)and [2022](https://publicholidays.co.id/2022-dates/).

                -   However, there are also days where it is a holiday but vaccination data is still being recorded such as 1 January 2022 (New Year's Day) and 1 May 2022(Labour Day). A possible reason could be that, assuming Indonesia is similar to Singapore, certain clinics will still be opened but only for selected hours.
...
In our 'data/aspatial' folder, we have multiple .xlsx files ranging from 1 July 2021 to 1 June 2022. But before we compile it togther, we need to understand each dataset and check for any discrepancies.

The code chunk below uses the following functions:
...
-   [glimpse():](https://www.rdocumentation.org/packages/dplyr/versions/0.3/topics/glimpse) to see every column in the dataframe

First, I stored each excel data in 1 variable.

```{r}


jkt2021_jul <- read_excel("data/aspatial/Data Vaksinasi Berbasis Kelurahan (01 Juli 2021).xlsx")

jkt2021_aug <- read_excel("data/aspatial/Data Vaksinasi Berbasis Kelurahan (1 Agustus 2021).xlsx")

jkt2021_feb <- read_excel("data/aspatial/Data Vaksinasi Berbasis Kelurahan (01 Februari 2022).xlsx")

#glimpse(jkt2021_jul1)

```

To get a better understanding of the data, we need to convert it from Indonesian language to Englishsian language to English
...
By using google translate, I am able to get the translations below:

\[1\] "SOLUTION CODE" "CITY AREA"

\[3\] "SUB-DISTRICT" "SUB-DISTRICT"

\[5\] "TARGET" "YET'S VACCINE"
...
\[15\] "TOTAL PUBLIC SERVICES\\r\\nVACCINES GRANTED" "GOTONG ROYONG\\r\\nDOSE 1"

\[17\] "GOTONG ROYONG\\r\\nDOSE 2" "TOTAL COLLECTION\\r\\nVACCINES WERE GIVEN"

\[19\] "HEALTH PERSONNEL\\r\\nDOSE 1" "HEALTH PERSONNEL\\r\\nDOSE 2"

\[21\] "TOTAL HEALTH PERSONNEL\\r\\nVACCINE GRANTED"

\_\_\_\_\_

The purpose of the code chunk below is to reach each excel file under the folder 'data/aspatial' , create its df and assign it to a variable name which is in the format of "month_year". E.g April_2022 .
...
for (x in study_period_data){
 ...
 # extract the year and month of the data file
...
Next, I would like to check whether there are any duplicate columns for each dataset.

1\) I created a function called duplication_check() which takes in the df filename as a variable.
...
2\) I made use of df_list which I populated earlier on to contain all of our df names and looped through it to determine which df has duplicated columns
...
This means that in our datasets with duplicated columns, there is 4 related columns - Elderly Dosage 1, Elderly Dosage 2, Elderly Dosage 3, Total Elderly Dosage where Dosage 3 values are a repeat of Dosage 2 avlues.
...
Next, I decided to check our missing values.
...
Based on the output, I realised that all the df were missing 3 fields. Looking deeper, I realised it was the same 3 fields (1st row, first 3 columns) that were blank. The reason why it is empty is due to the fact that is was merged with the top column in the excel and the row was meant to represent the total dosages across all district. Thus, we will be removing it later.

Next, we need to combine our dataset.

**What are the relevant columns?**

I would need to add the date and year in a new column

need to convert to english language for easier comprehension

```{r}

# we can derive count of people who were vacinated by target - yet to be vacinated
...
#translate the months into english
combined_df$TIME_PERIOD[combined_df$TIME_PERIOD == 'Desember_2021'] <- 'December_2021'
combined_df$TIME_PERIOD[combined_df$TIME_PERIOD == 'Februari_2022'] <- 'February_2022'
combined_df$TIME_PERIOD[combined_df$TIME_PERIOD == 'Januari_2022'] <- 'January_2022'
combined_df$TIME_PERIOD[combined_df$TIME_PERIOD == 'Juli_2021'] <- 'July_2021'
combined_df$TIME_PERIOD[combined_df$TIME_PERIOD == 'Juni_2022'] <- 'June_2022'
combined_df$TIME_PERIOD[combined_df$TIME_PERIOD == 'Mei_2022'] <- 'May_2022'
combined_df$TIME_PERIOD[combined_df$TIME_PERIOD == 'Oktober_2021'] <- 'October_2021'
combined_df$TIME_PERIOD[combined_df$TIME_PERIOD == 'Maret_2022'] <- 'March_2022'
combined_df$TIME_PERIOD[combined_df$TIME_PERIOD == 'Agustus_2021'] <- 'August_2021'
```
...
```{r}
# joins based on Province, Sub_District and City
...
#which(is.na(combined_jakarta))

#tmp <- combined_jakarta %>% 
...
# 5. Calculation: Monthly Vaccination Rates
...
```{r}
#sort it in ascending time order

reference = c("SUBDISTRICT","July_2021","August_2021","September_2021","October_2021","November_2021","December_2021","January_2022","February_2022","March_2022","April_2022","May_2022","June_2022")
...
# 6. Mapping of Monthly Vaccination Rates
...
var(vr$`August_2021`)
var(vr$`October_2021`)
var(vr$`December_2021`)
var(vr$`February_2022`)
var(vr$`June_2022`)

```

Instead, to get a clearer visualization of a specific month, I decided to use the classification method, cont as it creates a smooth, linear gradient in which the change in values is proportionally related to the change in colors. For the color palette, i decided to go with a sequential scheme from yellow to green as it more suited for ordered data that progress from low to high.

The code chunk below is an interactive map of vaccination rates in July 2021. By clicking on a certain location, it will show us the district name and respective vaccination rates. The darker the green, the higher the vaccination rates.

```{r}
  tmap_mode("view")
  tm_shape(vr)+
    tm_polygons("July_2021", 
            n= 6,
            style = "cont",
...
3.  Not much clusters can be identified.

4.  Sub-districts with lower vaccination rates can try to learn/adopt strategies from sub districts with higher vaccination rates' to increase their own vaccination rates.

The code chunk below is an interactive map of vaccination rates in June 2022.

```{r}
  tmap_mode("view")
  tm_shape(vr)+
    tm_polygons("June_2022", 
            n= 6,
            style = "cont",
            title = "Vaccination Rate(%)",
            palette = "YlGn",
...
4.  Thus, we can infer that within a span of 1 year, the vaccination rates across the sub-districts in Jakarta have increased.
...
1.  We can actually see Jakarta's vaccination rate increase steadily across the period of July 2021 to February 2022.
