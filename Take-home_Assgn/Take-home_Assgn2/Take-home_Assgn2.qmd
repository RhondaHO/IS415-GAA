...
The code chunk below creates a function called gi_plot() which takes in a date value and display the respective maps for that specific date with the gi\* values that is significant.
...
2.  Overall, we can observe that there is spatial clustering in our data. From July 2021 to June 2022, the no. of hot and cold spot areas have changed and that the intensity of Gi\* value has changed. The lowest range of Gi\* value decreased from range -4 to -2 to range -8 to -6.

3.  In October 2021, there is an unusual large number of hot and cold spots which signifies 5 significant clusters of hot spot areas with values higher than average and 7 significant clustersof cold spot areas with values lower than average.
